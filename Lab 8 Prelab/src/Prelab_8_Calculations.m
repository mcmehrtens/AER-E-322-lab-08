% AER E 322 Prelab 8 Spring 2023
% Matthew Mehrtens
% Section 4 Group 2
clear,clc;

% Formula for Moment of Inertia of Specimen 1 & 2
I = @(h, b, t) b * t^3 / 6 + b * t * h^2 / 2 ...
    + t * (h - t)^3 / 12; % [inch^4]
...
t_4 = 0.071; % [inch]
OD_4 = 1.66; % [inch]
theta_0_4 = 36.3 / 2; % [deg]
r_4 = r(OD_4, t_4); % [inch]

% Specimen 5 Parameters
t_5 = 0.071; % [inch]
OD_5 = 1.66; % [inch]
theta_0_5 = 103.7 / 2; % [deg]
r_5 = r(OD_5, t_5); % [inch]

% Shear center calculations
e_c = @(h, b, t, I) h^2 * b^2 * t / (4 * I); % [inch]
e_circ = @(r, theta_0) 2 * r ...
    * (cosd(theta_0) * (2 * pi - 2 * theta_0) + 2 * sind(theta_0)) ...
    / (2 * pi - 2 * theta_0 + sind(2 * theta_0)); % [inch]
